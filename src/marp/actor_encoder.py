...
class ActorEncoder:
    ...
    def fit(self, X: pd.DataFrame, y: pd.DataFrame) -> None:
        ...
        actor_scores_df['actor'] = actor_scores_df['actor'].apply(
            lambda x: str(x))
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(self.n_actors):
            df = df.reset_index()
            df[f'actor_{i}'] = df['cast'].apply(
                lambda x: str(x[i]) if len(x) > i else '~')
            df2 = pd.merge(df, self.actor_scores_df,
                           left_on=f'actor_{i}', right_on='actor')
            df2 = df2[df2['date_x'] > df2['date_y']].sort_values(
                ['actor', 'date_x']).groupby('index').last()[['avg_score']]
        ...
        for i in range(self.n_actors):
            df['actor_mean'] += df[f'actor_{i}'].apply(
                lambda x: x if x >= 0 else 0)
            df['n_scores'] += df[f'actor_{i}'].apply(
        ...
        for i in range(self.n_actors):
            df[f'actor_{i}'] = df[[
                f'actor_{x+i}' for x in range(self.n_actors-i)]].bfill(axis=1).iloc[:, 0]
